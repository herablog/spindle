...
## Style
<Description>ボタンは個別にスタイルのカスタマイズができます。</Description>
<Description>フォントサイズは16pxを基準にした相対値(`em`)で記述されているため、親要素のフォントサイズを指定することで変更できます。</Description>
<Description>また、カスタムプロパティを利用してスタイルを変更できます。変更できる項目は[ソースコードのカスタムプロパティ指定部分]()を変更方法は[プルリクエスト]]()を参照してください。</Description>
